...
export default defineComponent({
  ...
  methods: {
    handleHotkey(event) {
      // Check if the pressed key is "d" and if the event doesn't originate from an input element
      if (event.key === 'd' && !event.target.matches('input, textarea, select')) {
        // Trigger the dashboard navigation here
        this.menuSelected();
        this.$router.push({ name: 'admin.dashboard.index' });
      }
	  if (event.key === 'p' && !event.target.matches('input, textarea, select')) {
        // Trigger the dashboard navigation here
        this.menuSelected();
        this.$router.push({ name: 'admin.pos.index' });
      }
	  if (event.key === 'r' && !event.target.matches('input, textarea, select')) {
        // Trigger the dashboard navigation here
        this.menuSelected();
        this.$router.push({ name: 'admin.reports.sales_summary.index' });
      }
    },

    menuSelected() {
      // Implement your menuSelected logic here
    },
  },
});
</script>
